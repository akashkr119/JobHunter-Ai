...
BASE_DIR=Path(__file__).resolve().parent.parent;DATA_DIR=BASE_DIR/"data";LOG_DIR=BASE_DIR/"logs";DATABASE_DIR=BASE_DIR/"database";DATABASE_PATH=DATABASE_DIR/"jobs.db";USER_AGENT="JobHunterAI/1.0";REQUEST_TIMEOUT=30;ALERT_PRIORITIES={"apply_now","high","medium","low"};LOG_LEVELS={"DEBUG","INFO","WARNING","ERROR","CRITICAL"}
@dataclass(frozen=True)
class Settings:
    database_path:str=str(DATABASE_PATH);resume_path:str="resume.pdf";min_match_score:float=60.0;scheduler_hours:int=6;adzuna_app_id:str|None=None;adzuna_app_key:str|None=None;jooble_api_key:str|None=None;serpapi_key:str|None=None;source_limit:int=50;smtp_host:str|None=None;smtp_port:int=587;smtp_username:str|None=None;smtp_password:str|None=None;smtp_sender:str|None=None;email_recipient:str|None=None;telegram_bot_token:str|None=None;telegram_chat_id:str|None=None;notification_channel:str|None=None;notification_min_priority:str="apply_now";notification_min_recommendation_score:float=0.0;target_titles:tuple[str,...]=();preferred_locations:tuple[str,...]=();work_modes:tuple[str,...]=();desired_keywords:tuple[str,...]=();excluded_keywords:tuple[str,...]=();log_level:str="INFO";run_history_path:str|None=None;run_lock_path:str|None=None
    @classmethod
    def from_env(cls,env_file:str|None=None):
        load_dotenv(dotenv_path=env_file,override=False);settings=cls(database_path=os.getenv("JOBHUNTER_DATABASE_PATH",str(DATABASE_PATH)),resume_path=os.getenv("JOBHUNTER_RESUME_PATH","resume.pdf"),min_match_score=_float_env("JOBHUNTER_MIN_MATCH_SCORE",60.0),scheduler_hours=_int_env("JOBHUNTER_SCHEDULER_HOURS",6),adzuna_app_id=_optional_env("JOBHUNTER_ADZUNA_APP_ID"),adzuna_app_key=_optional_env("JOBHUNTER_ADZUNA_APP_KEY"),jooble_api_key=_optional_env("JOBHUNTER_JOOBLE_API_KEY"),serpapi_key=_optional_env("JOBHUNTER_SERPAPI_KEY"),source_limit=_int_env("JOBHUNTER_SOURCE_LIMIT",50),smtp_host=_optional_env("JOBHUNTER_SMTP_HOST"),smtp_port=_int_env("JOBHUNTER_SMTP_PORT",587),smtp_username=_optional_env("JOBHUNTER_SMTP_USERNAME"),smtp_password=_optional_env("JOBHUNTER_SMTP_PASSWORD"),smtp_sender=_optional_env("JOBHUNTER_SMTP_SENDER"),email_recipient=_optional_env("JOBHUNTER_EMAIL_RECIPIENT"),telegram_bot_token=_optional_env("JOBHUNTER_TELEGRAM_BOT_TOKEN"),telegram_chat_id=_optional_env("JOBHUNTER_TELEGRAM_CHAT_ID"),notification_channel=_optional_env("JOBHUNTER_NOTIFICATION_CHANNEL"),notification_min_priority=os.getenv("JOBHUNTER_NOTIFICATION_MIN_PRIORITY","apply_now").strip().lower() or "apply_now",notification_min_recommendation_score=_float_env("JOBHUNTER_NOTIFICATION_MIN_RECOMMENDATION_SCORE",0.0),target_titles=_csv_env("JOBHUNTER_TARGET_TITLES"),preferred_locations=_csv_env("JOBHUNTER_PREFERRED_LOCATIONS"),work_modes=_csv_env("JOBHUNTER_WORK_MODES"),desired_keywords=_csv_env("JOBHUNTER_DESIRED_KEYWORDS"),excluded_keywords=_csv_env("JOBHUNTER_EXCLUDED_KEYWORDS"),log_level=os.getenv("JOBHUNTER_LOG_LEVEL","INFO").strip().upper() or "INFO",run_history_path=_optional_env("JOBHUNTER_RUN_HISTORY_PATH"),run_lock_path=_optional_env("JOBHUNTER_RUN_LOCK_PATH"));settings.validate();return settings
    def validate(self):
        ...
        if self.source_limit<=0:raise ValueError("JOBHUNTER_SOURCE_LIMIT must be greater than zero")
        if not 1<=self.smtp_port<=65535:raise ValueError("JOBHUNTER_SMTP_PORT must be between 1 and 65535")
        if not 0<=self.notification_min_recommendation_score<=100:raise ValueError("JOBHUNTER_NOTIFICATION_MIN_RECOMMENDATION_SCORE must be between 0 and 100")
        if self.log_level not in LOG_LEVELS:raise ValueError("JOBHUNTER_LOG_LEVEL must be DEBUG, INFO, WARNING, ERROR or CRITICAL")
    ...
    def notification_config(self):
        ...
        config={"minimum_priority":self.notification_min_priority,"minimum_recommendation_score":self.notification_min_recommendation_score}
        if channel=="telegram":
