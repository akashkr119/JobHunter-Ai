...
class Scheduler:
    ...
    def run_pipeline(
        ...
        """Scrape, score and persist jobs from supported ATS career URLs.

        A failure on one career URL is recorded and does not prevent the other
        companies from being processed.
        """
        ...
        for career_url in career_urls:
            ...
            try:
                ...
                summary["errors"].append(
                    {"career_url": career_url, "error": str(exc)}
                )
                continue
