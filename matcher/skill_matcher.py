...
class SkillMatcher:
    ...
    def classify_job_skills(self, job_description: str) -> dict[str, list[str]]:
        """Classify detected skills as required, preferred, or general.

        Classification is deliberately deterministic: section headings and
        requirement phrases influence weighting without relying on an external
        AI service. Unclassified skills remain general requirements.
        """
    ...
    def match(
        ...
        """Return deterministic match score and supporting skill details."""
        ...
        if job_skills is not None:
            ...
            preferred = set(groups["preferred"])
            general = set(groups["general"])
    ...
    def _weighted_score(resume, required, preferred, general) -> float:
        """Weight required skills highest while preserving legacy behavior."""
        ...
        total = (
            len(required) * weights["required"]
            + len(general) * weights["general"]
            + len(preferred) * weights["preferred"]
        )
        earned = (
            len(resume & required) * weights["required"]
            + len(resume & general) * weights["general"]
            + len(resume & preferred) * weights["preferred"]
        )
