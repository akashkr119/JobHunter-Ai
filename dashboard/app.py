...
def _list_jobs(min_score=0.0,limit=100,status=None):
    db=Database(_database_path())
    try:return db.list_jobs(min_score=min_score,limit=limit,status=status)
    finally:db.close()
def _job_summary(job):
    keys=("id","title","company","location","platform","match_score","matched_skills","missing_skills","required_skills","preferred_skills","matched_required_skills","missing_required_skills","application_status","status_updated_at","apply_url","discovered_at","updated_at")
...
def jobs():
    ...
    try:recommendations=[_job_summary(j) for j in _list_jobs(min_score,limit,status)]
