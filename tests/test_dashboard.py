...
def test_analytics_endpoint_empty_database(tmp_path,monkeypatch):
    path=tmp_path/"empty.db";Database(path).close();monkeypatch.setenv("JOBHUNTER_DATABASE_PATH",str(path));a=app.test_client().get("/api/analytics").get_json();assert a["total"]==0;assert a["saved"]==0;assert a["average_match_score"]==0.0
...
def test_jobs_endpoint_returns_tracking_fields(tmp_path,monkeypatch):
    path=tmp_path/"dashboard.db";_seed_database(path);monkeypatch.setenv("JOBHUNTER_DATABASE_PATH",str(path));payload=app.test_client().get("/api/jobs").get_json();job=payload["jobs"][0];assert payload["count"]==2;assert job["title"]=="QA Automation Engineer";assert job["is_saved"] is False;assert job["notes"]=="";assert job["missing_required_skills"]==["docker"]
...
def test_saved_query_validation(tmp_path,monkeypatch):
    path=tmp_path/"dashboard.db";_seed_database(path);monkeypatch.setenv("JOBHUNTER_DATABASE_PATH",str(path));r=app.test_client().get("/api/jobs?saved=maybe");assert r.status_code==400;assert r.get_json()["error"]=="saved must be true or false"
