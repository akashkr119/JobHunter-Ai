...
def test_homepage_renders_visual_dashboard():
    response=app.test_client().get("/"); html=response.get_data(as_text=True); assert response.status_code==200; assert "JobHunter AI" in html; assert 'id="search"' in html; assert 'id="score"' in html; assert 'id="platform"' in html; assert 'id="status"' in html; assert "/api/jobs?limit=500" in html; assert "/status" in html; assert "Apply" in html; assert "Missing required" in html
...
def test_status_update_and_filter_workflow(tmp_path,monkeypatch):
    path=tmp_path/"dashboard.db"; job_id=_seed_database(path); monkeypatch.setenv("JOBHUNTER_DATABASE_PATH",str(path)); client=app.test_client()
    response=client.patch(f"/api/jobs/{job_id}/status",json={"status":"applied"}); payload=response.get_json(); assert response.status_code==200; assert payload["application_status"]=="applied"; assert payload["status_updated_at"] is not None
    filtered=client.get("/api/jobs?status=applied").get_json(); assert filtered["count"]==1; assert filtered["jobs"][0]["id"]==job_id
    assert client.get("/api/jobs?status=new").get_json()["count"]==1


def test_status_update_validation(tmp_path,monkeypatch):
    path=tmp_path/"dashboard.db"; job_id=_seed_database(path); monkeypatch.setenv("JOBHUNTER_DATABASE_PATH",str(path)); client=app.test_client()
    missing=client.patch(f"/api/jobs/{job_id}/status",json={}); assert missing.status_code==400; assert missing.get_json()["error"]=="status is required"
    invalid=client.patch(f"/api/jobs/{job_id}/status",json={"status":"hired"}); assert invalid.status_code==400; assert "allowed_statuses" in invalid.get_json()
    assert client.patch("/api/jobs/999/status",json={"status":"applied"}).status_code==404
    assert client.get("/api/jobs?status=hired").status_code==400
