...
def test_career_finder_instance():
    assert CareerFinder() is not None


def test_has_find_method():
    assert hasattr(CareerFinder(), "find")


def test_find_method_is_callable():
    assert callable(CareerFinder().find)


def test_candidate_urls_include_common_paths():
    urls = CareerFinder().candidate_urls("example.com")
...
def test_discover_finds_internal_and_external_career_links():
    ...
    response.text = """
        <html><body>
          ...
          <a href="https://jobs.lever.co/example">Open positions</a>
          <a href="/about">About us</a>
          <a href="mailto:hr@example.com">Jobs email</a>
        </body></html>
    """
    session.get.return_value = response
    finder = CareerFinder(session=session)

    urls = finder.discover("https://example.com")

    response.raise_for_status.assert_called_once()
    assert "https://example.com/careers" in urls
    assert "https://jobs.lever.co/example" in urls
...
def test_discover_resolves_relative_job_link():
    session = MagicMock()
    response = MagicMock()
    response.text = '<a href="/company/jobs/search">Find Jobs</a>'
    session.get.return_value = response

    urls = CareerFinder(session=session).discover("https://example.com")

    assert urls == ["https://example.com/company/jobs/search"]


def test_find_discovery_places_real_links_before_fallbacks():
    session = MagicMock()
    response = MagicMock()
    response.text = '<a href="https://boards.greenhouse.io/example">Careers</a>'
    session.get.return_value = response
...
def test_find_falls_back_when_homepage_request_fails():
    ...
    urls = finder.find("https://example.com", discover=True)
...
def test_find_without_discovery_does_not_make_network_request():
    ...
    urls = finder.find("example.com", discover=False)
...
def test_discover_uses_configured_timeout_and_user_agent():
    ...
    session.get.return_value = response
    finder = CareerFinder(session=session, timeout=7)

    finder.discover("https://example.com")
...
def test_empty_website_rejected(value):
    with pytest.raises(ValueError, match="Website URL cannot be empty"):
        CareerFinder().candidate_urls(value)
