...
def test_smart_alert_can_require_recommendation_score(tmp_path):
    url="https://boards.greenhouse.io/example";n=Notifier();db=Database(tmp_path/"jobs.db");s=Scheduler(scraper_factory=Factory([make_job("https://example.com/1","QA Automation Engineer")]),matcher=SkillMatcher(),database=db,notifier=n);summary=s.run_pipeline([url],["python","selenium"],notification={"channel":"telegram","minimum_priority":"low","minimum_recommendation_score":100});assert summary["jobs_saved"]==1;assert summary["notifications_sent"]==0;assert summary["notifications_suppressed"]==1;assert n.sent==[];db.close()
