...
def test_save_and_get_job(tmp_path):
    ...
    job_id = db.save_job(job, match={"score":75.0,"matched_skills":["python","selenium","pytest"],"missing_skills":["docker"],"required_skills":["python","selenium"],"preferred_skills":["docker"],"general_skills":["pytest"],"matched_required_skills":["python","selenium"],"missing_required_skills":[]})
    stored = db.get_job(job_id)
    assert stored["match_score"] == 75.0; assert stored["matched_skills"] == ["python","selenium","pytest"]; assert stored["missing_skills"] == ["docker"]
    assert stored["required_skills"] == ["python","selenium"]; assert stored["preferred_skills"] == ["docker"]; assert stored["general_skills"] == ["pytest"]
    assert stored["matched_required_skills"] == ["python","selenium"]; assert stored["missing_required_skills"] == []; assert stored["platform"] == "greenhouse"; assert stored["location"] == "Bengaluru"
    assert stored["application_status"] == "new"; db.close()


def test_duplicate_apply_url_updates_existing_job(tmp_path):
    db = Database(tmp_path / "jobs.db"); url = "https://example.com/jobs/1"
    ...
    jobs = db.list_jobs(); assert len(jobs)==1; assert first_id==second_id; assert jobs[0]["title"]=="Senior QA Engineer"; assert jobs[0]["location"]=="Bengaluru"; assert jobs[0]["match_score"]==90.0; assert jobs[0]["missing_required_skills"]==["docker"]; db.close()


def test_rescrape_preserves_application_status(tmp_path):
    db=Database(tmp_path/"jobs.db"); url="https://example.com/jobs/tracked"
    job_id=db.save_job({"title":"QA Engineer","company":"Example","apply_url":url}, match={"score":70})
    db.update_application_status(job_id,"applied")
    second_id=db.save_job({"title":"Senior QA Engineer","company":"Example","apply_url":url,"location":"Bengaluru"}, match={"score":92})
    stored=db.get_job(job_id); assert second_id==job_id; assert stored["application_status"]=="applied"; assert stored["match_score"]==92.0; assert stored["title"]=="Senior QA Engineer"; db.close()


def test_update_and_filter_application_status(tmp_path):
    db=Database(tmp_path/"jobs.db")
    a=db.save_job({"title":"A","company":"Example","apply_url":"https://example.com/a"},match={"score":80})
    db.save_job({"title":"B","company":"Example","apply_url":"https://example.com/b"},match={"score":90})
    updated=db.update_application_status(a,"interview"); assert updated["application_status"]=="interview"; assert updated["status_updated_at"] is not None
    jobs=db.list_jobs(status="interview"); assert len(jobs)==1; assert jobs[0]["id"]==a; db.close()
...
def test_existing_database_is_migrated_without_losing_jobs(tmp_path):
    ...
    conn.execute("""CREATE TABLE jobs (id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT NOT NULL,company TEXT NOT NULL,location TEXT NOT NULL DEFAULT '',apply_url TEXT NOT NULL UNIQUE,description TEXT NOT NULL DEFAULT '',platform TEXT NOT NULL DEFAULT 'unknown',match_score REAL NOT NULL DEFAULT 0,matched_skills TEXT NOT NULL DEFAULT '[]',missing_skills TEXT NOT NULL DEFAULT '[]',discovered_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)""")
    conn.execute("INSERT INTO jobs (title,company,apply_url) VALUES (?,?,?)",("Legacy QA","Example","https://example.com/legacy")); conn.commit(); conn.close()
    db=Database(path); columns={row["name"] for row in db.fetchall("PRAGMA table_info(jobs)")}; assert {"required_skills","preferred_skills","general_skills","matched_required_skills","missing_required_skills","application_status","status_updated_at"}.issubset(columns); jobs=db.list_jobs(); assert len(jobs)==1; assert jobs[0]["title"]=="Legacy QA"; assert jobs[0]["missing_required_skills"]==[]; assert jobs[0]["application_status"]=="new"; db.close()
