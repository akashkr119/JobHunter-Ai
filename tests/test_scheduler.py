...
def test_high_threshold_can_alert_high_priority_job(tmp_path):
    s=build_scheduler(tmp_path,notifier=FakeNotifier());job={"is_active":True,"application_status":"new","priority_label":"high","last_notified_priority":""};assert s._should_notify(job,None,{"minimum_priority":"high"}) is True;assert s._should_notify(job,None,{"minimum_priority":"apply_now"}) is False;s.database.close()
