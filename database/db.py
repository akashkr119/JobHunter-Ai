...
class Database:
    ...
    def save_job(self,job,match=None):
        ...
        match=match or {};lists={n:list(match.get(n) or []) for n in self.MATCH_LIST_COLUMNS};preference_score=float(match.get("preference_score",100));preference_match=1 if match.get("preference_match",True) else 0;preference_details=json.dumps(match.get("preference_details") or {});values=(t,co,loc,u,platform,float(match.get("score",0)),preference_score,preference_match,preference_details,str(d.get("description") or "").strip(),key,*(json.dumps(lists[n]) for n in self.MATCH_LIST_COLUMNS));existing=self.conn.execute("SELECT id,apply_url FROM jobs WHERE apply_url=? OR (job_key=? AND job_key<>'') ORDER BY CASE WHEN apply_url=? THEN 0 ELSE 1 END,id LIMIT 1",(u,key,u)).fetchone()
        if existing:
            aliases={r["apply_url"] for r in self.conn.execute("SELECT apply_url FROM job_aliases WHERE job_id=?",(existing["id"],)).fetchall()};aliases.update((existing["apply_url"],u));self.execute("""UPDATE jobs SET title=?,company=?,location=?,apply_url=?,platform=?,match_score=?,preference_score=?,preference_match=?,preference_details=?,description=?,job_key=?,matched_skills=?,missing_skills=?,required_skills=?,preferred_skills=?,general_skills=?,matched_required_skills=?,missing_required_skills=?,last_seen_at=CURRENT_TIMESTAMP,is_active=1,updated_at=CURRENT_TIMESTAMP WHERE id=?""",(*values,int(existing["id"])))
            for alias in aliases:self.execute("INSERT OR IGNORE INTO job_aliases(job_id,apply_url) VALUES(?,?)",(int(existing["id"]),alias))
            return int(existing["id"])
        c=self.execute("""INSERT INTO jobs(title,company,location,apply_url,platform,match_score,preference_score,preference_match,preference_details,description,job_key,matched_skills,missing_skills,required_skills,preferred_skills,general_skills,matched_required_skills,missing_required_skills,last_seen_at,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,1)""",values);job_id=int(c.lastrowid);self.execute("INSERT OR IGNORE INTO job_aliases(job_id,apply_url) VALUES(?,?)",(job_id,u));return job_id
    ...
    def _row_to_dict(cls,row):
        ...
        try:r["preference_details"]=json.loads(r.get("preference_details") or "{}")
        except (TypeError,json.JSONDecodeError):r["preference_details"]={}
        r["preference_match"]=bool(r.get("preference_match",1));r["is_saved"]=bool(r.get("is_saved",0));r["is_active"]=bool(r.get("is_active",1));r["follow_up_completed"]=bool(r.get("follow_up_completed",0));r.update(cls._follow_up(r));r["priority_score"]=cls._priority(r);r["priority_label"]=cls._priority_label(r["priority_score"]);return r
